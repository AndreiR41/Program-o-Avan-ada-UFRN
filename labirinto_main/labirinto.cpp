...
double Labirinto::calculaCaminho(int& prof, int& NAbert, int& NFech)
{
 ...
 //aqui usa mov para indicar a direção (movimento que vai fazer)
  ...
  do {
     ...
     if (atual.pos != dest) {
         ...
         for (mov.lin = -1; mov.lin <= 1; mov.lin++)
         {
             ...
             {     //se mov diferente (0,0), não é válida
                 if (mov != Coord(0, 0)) {
                     prox = atual.pos + mov;
                     ...
                     if (movimentoValido(atual.pos, prox)) {
                         ...
                         //Custo passado
                         suc.g = atual.g + (th / 2) + modulo(mov); //como função solta
                         // Estimativa custo futuro
                         suc.h = prox.heuristicaDiag(dest);
                         //custo diferencial por vizinhança (8 vizinhos)
                         ...
                         //Assume que não existe nó igual ao sucessor
                         bool eh_inedito = true;
                         // Procura em Aberto Noh com pos igual a possuc
                         auto old = find(Abertos.begin(), Abertos.end(), suc.pos);
                         // se EXISTE old em aberto(quando usa o find e não encontra nada aponta pro .end)
                         if (old != Abertos.end()) {
                             ...
                             // procura em fechado nó igual a pos suc(mesma posição que suc)
                         ...
                         //Insere em aberto se não exsite nem em aberto nem em fechado
                         if (eh_inedito) {
                             //acha big,o 1º Nó de aberto com custo total< suc.custoTotal
     ...
     //disponibiliza o estado final da busca ,quer encontre ou não o caminho
         NFech = Fechados.size();
         NAbert = Abertos.size();
         
     // Pode ter terminado porque encontrou a solução ou porque não há mais nós 
         if (atual.pos != dest) {
         //Disponibiliza a informação que não existe caminho
             compr = -1.0;
             prof = -1;
         }
         else {
         //calcula depois disponibiliza comrpimento e profundidade
             ...
             {
                 // Assinala a célula como pertencente ao caminho
